...
### Without a packet library, it fails closed

On a standalone server with neither PacketEvents nor ProtocolLib, nothing can send the
encryption request. A `REAL` account then lands in limbo asking for `/login`, and since it
has no password, **nobody** gets in — not an impostor, and not the owner either.

Inconvenient, but not insecure. Install one of the two libraries, or convert the account
with `/offline` so it uses a password.
...
- the server is standalone with no packet library
...
<Callout title="The question solves both">
  With `enable: false` nobody is refused, and a real owner reaches the same place one click
  later thanks to the question below. See [Who gets a name first](#who-gets-a-name-first).
</Callout>
...
- **Yes** converts the account to premium and asks them to reconnect. From then on they join
  without a password, permanently.
...
## Standalone servers need a packet library

On Velocity this works out of the box, because the proxy owns the login handshake.

A Paper server with **no proxy** needs [PacketEvents](https://github.com/retrooper/packetevents)
or [ProtocolLib](https://github.com/dmulloy2/ProtocolLib), because with `online-mode=false`
the server never asks anyone to authenticate with Mojang and Paper offers no way to ask just
one player to — those libraries provide it.

Without one, the console says so at startup and premium players fall back to `/register`.
