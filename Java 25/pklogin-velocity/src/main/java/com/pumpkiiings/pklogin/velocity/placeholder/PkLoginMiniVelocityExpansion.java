package com.pumpkiiings.pklogin.velocity.placeholder;
...
public final class PkLoginMiniVelocityExpansion {

    private static final String DATE_FORMAT = "yyyy-MM-dd HH:mm";

    private PkLoginMiniVelocityExpansion() {}

    public static void register(PkLoginVelocity plugin) {
        ...
        Expansion.builder("pklogin")
            ...
            .audiencePlaceholder(Player.class, "is_logged_in", (player, queue, ctx) ->
                Tag.preProcessParsed(bool(plugin.isAuthenticated(player.getUsername()))))

            .audiencePlaceholder(Player.class, "is_registered", (player, queue, ctx) -> {
                Optional<Account> acc = accountMgr.retrieveOrLoad(player.getUsername());
            ...
            .audiencePlaceholder(Player.class, "account_type", (player, queue, ctx) -> {
                Optional<Account> acc = accountMgr.retrieveOrLoad(player.getUsername());
                return Tag.preProcessParsed(acc.map(PkLoginMiniVelocityExpansion::resolveType).orElse("UNKNOWN"));
            })
